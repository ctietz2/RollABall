...
namespace Valve.VR.InteractionSystem
{
	...
	public class TeleportPoint : TeleportMarkerBase
	{
		...
		// This function is called when the player teleports
		private void OnPlayerTeleported(TeleportMarkerBase teleportMarker)
		{
			if (teleportMarker == this) // Check if this is the destination
			{
				MovePlayer move = player.GetComponent<MovePlayer>();
				if (move != null)
				{
					move.SetTeleportPoint(this.transform);
				}
			}
		}

		// Ensure to remove the listener when this object is destroyed
		void OnDestroy()
		{
